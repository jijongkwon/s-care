...
class LogoutListenerService: WearableListenerService() {
    override fun onDataChanged(dataEvents: DataEventBuffer) {
        dataEvents.forEach { event ->
            if (event.type == DataEvent.TYPE_CHANGED && event.dataItem.uri.path == "/logoutStatus") {
                val dataMap = DataMapItem.fromDataItem(event.dataItem).dataMap
                val isLoggedIn = dataMap.getBoolean("isLoggedIn")

                // SharedPreferences나 DataStore에 저장
                saveLoginStatus(this, isLoggedIn)
                Log.d("LogoutListenerService", "Received login status: $isLoggedIn ")
            }
        }
    }
